...
17. `/Users/jhyang/.codex/sessions/2026/05/19/rollout-2026-05-19T15-40-39-019e3f2e-000a-7421-b249-34381903d9e0.jsonl`

  A2 安装验证记录显示 3 个工程事实: `profiling/prof_api.h` 实际位于 CANN `aarch64-linux/pkg_inc`, vLLM-Ascend custom op 可见性需要 `enable_custom_op()` 和加载 `vllm_ascend.vllm_ascend_C`, `SOC_VERSION=ascend910b1` 构建不会把 `dispatch_gmm_combine_decode` 的 Ascend Computing Language Neural Network (ACLNN) op_api 打入 custom ops 包, A3 应使用 `SOC_VERSION=ascend910_93`.
